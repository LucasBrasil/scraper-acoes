...
def main():
    ...
    for i, (idx, ticker, dados) in enumerate(tickers_dados, 1):
        try:
            ...
            print(f"  ROE%: {dados['roe']:.2f}  (SEM %)")
            print(f"  Marg%: {dados['marg']:.2f}  (SEM %)")
            print(f"  Result 12M%: {dados['res_12m']:.2f}  (SEM %)")
            print(f"  Osc 12M%: {dados['osc_12m']:.2f}  (SEM %)")
            print(f"  %Div: {dados['div']:.2f}  (SEM %)")
